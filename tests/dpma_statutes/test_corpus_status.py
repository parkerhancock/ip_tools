"""Tests for the DPMA Germany statutes ``get_corpus_status()`` callable."""
...
from patent_client_agents.dpma_statutes import get_corpus_status


class TestShape:
    def test_returns_dict_with_two_keys(self, dpma_statutes_corpus_env: Path) -> None:
...
class TestFromBundledCorpus:
    def test_version_falls_back_to_snapshot_label(self, dpma_statutes_corpus_env: Path) -> None:
        status = get_corpus_status()
    ...
    def test_synced_at_utc_normalized_to_midnight(self, dpma_statutes_corpus_env: Path) -> None:
        status = get_corpus_status()
...
class TestMissing:
    def test_returns_unknown_when_corpus_missing(
        ...
        monkeypatch.setenv("DPMA_STATUTES_CORPUS_PATH", str(tmp_path / "missing.db"))
        status = get_corpus_status()
    ...
    def test_source_version_preferred_when_present(
        ...
        from patent_client_agents.dpma_statutes.corpus.schema import DDL
        ...
        try:
            ...
            conn.executemany(
                ...
                [
                    ...
                    ("source_version", "PatG-as-of-2024-08-01"),
                ],
            )
            conn.commit()
        finally:
        ...
        monkeypatch.setenv("DPMA_STATUTES_CORPUS_PATH", str(db_path))
        status = get_corpus_status()
        assert status["corpus_version"] == "PatG-as-of-2024-08-01"
