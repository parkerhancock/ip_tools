...
class RegisterSearchResult(BaseModel):
    """A single result from EP Register search."""
...
class RegisterSearchResponse(BaseModel):
    """Response from EP Register search."""

    query: str | None = None
    range_begin: int | None = None
    range_end: int | None = None
    total_results: int | None = None
    results: list[RegisterSearchResult] = Field(default_factory=list)


class DesignatedState(BaseModel):
    """A designated contracting state in an EP application."""
...
class ProceduralStep(BaseModel):
    """A procedural step in EP prosecution history."""
...
class OppositionParty(BaseModel):
    """A party in opposition proceedings."""

    name: str | None = None
    country: str | None = None
    role: str | None = None  # "opponent" or "patent_owner"


class OppositionData(BaseModel):
    """Opposition proceeding data."""

    opposition_date: str | None = None
    status: str | None = None
    parties: list[OppositionParty] = Field(default_factory=list)


class UnitaryPatentData(BaseModel):
    """Unitary Patent (UPP) data."""

    upp_status: str | None = None
    registration_date: str | None = None
    participating_states: list[str] = Field(default_factory=list)


class RegisterBiblioResponse(BaseModel):
    """Bibliographic data from EP Register."""
    ...
    designated_states: list[DesignatedState] = Field(default_factory=list)
    status: str | None = None
    status_description: str | None = None
    opposition: OppositionData | None = None


class RegisterEventsResponse(BaseModel):
    """Events from EP Register."""

    application_number: str | None = None
    events: list[RegisterEvent] = Field(default_factory=list)


class RegisterProceduralStepsResponse(BaseModel):
    """Procedural steps from EP Register."""

    application_number: str | None = None
    procedural_steps: list[ProceduralStep] = Field(default_factory=list)


class RegisterUppResponse(BaseModel):
    """Unitary Patent data from EP Register."""

    application_number: str | None = None
    unitary_patent: UnitaryPatentData | None = None
