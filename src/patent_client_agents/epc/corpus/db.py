...
class CorpusUnavailable(RuntimeError):
    """Raised when the EPC corpus database cannot be located or opened."""
...
def _resolve_corpus_path(explicit: str | os.PathLike[str] | None) -> Path:
    ...
    env = os.environ.get("EPC_CORPUS_PATH")
    if env:
        return Path(env)
...
_INSTALL_HINT = (
    "Run `patent-client-agents-build-epc-corpus --output "
    "~/.cache/patent_client_agents/epc.db` to build it, or set "
    "EPC_CORPUS_PATH to an existing corpus file."
)
...
class CorpusDB:
    ...
    def open(
        ...
        if must_exist and not resolved.exists():
            raise CorpusUnavailable(f"EPC corpus not found at {resolved}. {_INSTALL_HINT}")
        try:
            conn = sqlite3.connect(f"file:{resolved}?mode=ro", uri=True)
        except sqlite3.OperationalError as exc:
            raise CorpusUnavailable(
                f"Could not open EPC corpus at {resolved}: {exc}. {_INSTALL_HINT}"
            ) from exc
