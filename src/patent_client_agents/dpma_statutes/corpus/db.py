...
class CorpusUnavailable(RuntimeError):
    """Raised when the DPMA statutes corpus cannot be located or opened."""
...
def _resolve_corpus_path(explicit: str | os.PathLike[str] | None) -> Path:
    ...
    env = os.environ.get("DPMA_STATUTES_CORPUS_PATH")
    if env:
        return Path(env)
...
_INSTALL_HINT = (
    "Run `patent-client-agents-build-dpma-statutes-corpus --output "
    "~/.cache/patent_client_agents/dpma_statutes.db` to build it, or set "
    "DPMA_STATUTES_CORPUS_PATH to an existing corpus file."
)
    ...
    def open(
        ...
        if must_exist and not resolved.exists():
            raise CorpusUnavailable(
                f"DPMA statutes corpus not found at {resolved}. {_INSTALL_HINT}"
            )
        try:
            conn = sqlite3.connect(f"file:{resolved}?mode=ro", uri=True)
        except sqlite3.OperationalError as exc:
            raise CorpusUnavailable(
                f"Could not open DPMA statutes corpus at {resolved}: {exc}. {_INSTALL_HINT}"
            ) from exc
