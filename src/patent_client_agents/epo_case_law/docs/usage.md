...
`CASELAW_CORPUS_PATH` in the runtime env to point at it; the file does
not have to live in `~/.cache`.

## Search

* `CaseLawClient.search(query, syntax="adj", sort="relevance",
...
* `CaseLawClient.get_section(section)` accepts either a PA 1977 section
  ...
  or the full path `/guidance/manual-of-patent-practice-caselaw/...`).
* The returned `CaseLawSection.html` is the cleaned outerHTML of the
  page's `<main>` element with form/sidebar/feedback widgets stripped.
  `.text` is the same content with whitespace collapsed.

## Versions

`CaseLawClient.list_versions()` returns a single-entry list reflecting
...
If neither `CASELAW_CORPUS_PATH` nor `~/.cache/patent_client_agents/caselaw.db`
exists, the first call raises
`patent_client_agents.ukipo_caselaw.corpus.CorpusUnavailable` with the
