...
from .client import CaseLawClient
from .models import CaseLawSearchResponse, CaseLawSection, CaseLawVersion
...
class SectionInput(BaseModel):
    """Input for getting an CASELAW section.
...
def get_client() -> CaseLawClient:
    """Create an CaseLawClient.
...
def _warn_client_deprecated() -> None:
    warnings.warn(
        ...
        "Use 'async with CaseLawClient() as client:' instead.",
        DeprecationWarning,
        stacklevel=3,
    )
...
async def search(
    params: SearchInput,
    *,
    client: CaseLawClient | None = None,
) -> CaseLawSearchResponse:
    """Search the CASELAW.
    ...
    async with CaseLawClient() as cl:
...
async def get_section(
    ...
    *,
    ...
    """Get a specific CASELAW section.

    Args:
        params: Either a SectionInput object or a section number/href string.
            Section numbers (e.g., "2106", "2106.04(a)") are automatically
            resolved to hrefs.
        client: Deprecated. Use CaseLawClient as context manager instead.

    Returns:
        CaseLawSection with content.
...
async def list_versions(
    *,
    ...
    """List available CASELAW versions.
