...
produced by ``patent-client-agents-build-caselaw-corpus``. The database
is not bundled with the wheel — deployments materialize it via the
build CLI and the runtime locates it through:

1. ``CASELAW_CORPUS_PATH`` environment variable (explicit path)
2. ``~/.cache/patent_client_agents/caselaw.db`` (local-dev default)
