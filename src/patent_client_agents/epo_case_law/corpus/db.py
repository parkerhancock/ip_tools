"""Read-side API for the CASELAW SQLite corpus.
...
class CorpusUnavailable(RuntimeError):
    """Raised when the CASELAW corpus database cannot be located or opened."""
...
def _resolve_corpus_path(explicit: str | os.PathLike[str] | None) -> Path:
    ...
    env = os.environ.get("CASELAW_CORPUS_PATH")
    if env:
        return Path(env)
...
_INSTALL_HINT = (
    "Run `patent-client-agents-build-caselaw-corpus --output "
    "~/.cache/patent_client_agents/caselaw.db` to build it, or set "
    "CASELAW_CORPUS_PATH to an existing corpus file."
)
...
class CorpusDB:
    ...
    def open(
        ...
        if must_exist and not resolved.exists():
            raise CorpusUnavailable(f"CASELAW corpus not found at {resolved}. {_INSTALL_HINT}")
        try:
            conn = sqlite3.connect(f"file:{resolved}?mode=ro", uri=True)
        except sqlite3.OperationalError as exc:
            raise CorpusUnavailable(
                f"Could not open CASELAW corpus at {resolved}: {exc}. {_INSTALL_HINT}"
            ) from exc
