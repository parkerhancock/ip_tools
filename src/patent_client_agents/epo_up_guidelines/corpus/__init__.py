...
produced by ``patent-client-agents-build-up-guidelines-corpus``. The database
is not bundled with the wheel — deployments materialize it via the
build CLI and the runtime locates it through:

1. ``UP_GUIDELINES_CORPUS_PATH`` environment variable (explicit path)
2. ``~/.cache/patent_client_agents/up_guidelines.db`` (local-dev default)
