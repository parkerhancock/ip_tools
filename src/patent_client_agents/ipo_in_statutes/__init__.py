"""IPO India statutes connector (MCP-free public surface).

The four core Indian IP Acts plus the Patent Rules — bundled into one
SQLite/FTS5 corpus discriminated by ``statute_name``. Static corpus
(``category=substantive_law``, ``transport=mcp_local``,
``update_strategy=scheduled_recrawl``, cadence ``annual``).
"""
...
class CorpusStatus(TypedDict):
    ...
    label: India's statutes are published as PDFs on indiacode.nic.in
    and ipindia.gov.in without a discrete vendor revision tag, so when
    no ``meta.source_version`` is stamped we fall back to a
...
def get_corpus_status() -> CorpusStatus:
    """Return IPO India statutes corpus freshness metadata for the validator and Provenance helper.

    Reads ``meta.snapshot_date`` and ``meta.source_version`` from the
    bundled SQLite corpus
    (:mod:`patent_client_agents.ipo_in_statutes.corpus.schema`). Does
    ...
    The corpus is located via ``IPO_IN_STATUTES_CORPUS_PATH`` or the
    local-dev default at
    ``~/.cache/patent_client_agents/ipo_in_statutes.db``. If the file
    ...
    try:
        ...
        _logger.debug("IPO India statutes corpus unavailable for get_corpus_status: %s", exc)
        ...
        _logger.warning(
            "IPO India statutes get_corpus_status: unexpected error reading corpus meta: %r",
            exc,
        )
        return CorpusStatus(corpus_synced_at=None, corpus_version="unknown")
...
def _parse_snapshot_date(value: str | None) -> datetime | None:
    ...
    try:
        ...
        _logger.debug(
            "IPO India statutes get_corpus_status: snapshot_date %r is not ISO date",
            value,
        )
        return None
    return datetime(
