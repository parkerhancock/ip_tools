...
Note: One-shot functions require ``KIPO_KIPRIS_API_KEY`` and ``KIPO_KIPRIS_API_KEY``
...
async def get_patent_progress(
    ...
    *,
    ...
    Args:
        ...
        username: JPO username (falls back to ``KIPO_KIPRIS_API_KEY`` env var).
        password: JPO password (falls back to ``KIPO_KIPRIS_API_KEY`` env var).

    Returns:
        Patent progress data or ``None`` if not found.
    """
    async with KiprisClient(username=username, password=password) as client:
