...
Usage:
    async with KiprisClient(username="...", password="...") as client:
        progress = await client.get_patent_progress("2020123456")

Environment Variables:
    KIPO_KIPRIS_API_KEY: JPO-issued username/ID
    KIPO_KIPRIS_API_KEY: JPO-issued password
"""
...
class KiprisClient(BaseAsyncClient):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            username: JPO-issued username. Falls back to ``KIPO_KIPRIS_API_KEY``.
            password: JPO-issued password. Falls back to ``KIPO_KIPRIS_API_KEY``.
        ...
        """
        resolved_username = username or os.getenv("KIPO_KIPRIS_API_KEY")
        resolved_password = password or os.getenv("KIPO_KIPRIS_API_KEY")

        if not resolved_username or not resolved_password:
            raise ConfigurationError(
                "JPO API credentials required. "
                "Set KIPO_KIPRIS_API_KEY and KIPO_KIPRIS_API_KEY environment variables "
