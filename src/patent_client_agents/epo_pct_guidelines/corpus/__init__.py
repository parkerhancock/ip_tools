...
produced by ``patent-client-agents-build-pct-guidelines-corpus``. The database
is not bundled with the wheel — deployments materialize it via the
build CLI and the runtime locates it through:

1. ``PCT_GUIDELINES_CORPUS_PATH`` environment variable (explicit path)
2. ``~/.cache/patent_client_agents/pct_guidelines.db`` (local-dev default)
