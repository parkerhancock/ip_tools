"""Corpus-backed EPO PCT-EPO Guidelines client.

Reads from a SQLite/FTS5 snapshot produced by
``patent-client-agents-build-pct-guidelines-corpus`` and located via
``PCT_GUIDELINES_CORPUS_PATH`` or ``~/.cache/patent_client_agents/pct_guidelines.db``.

The corpus is scraped from ``www.epo.org/en/legal/guidelines-pct/<year>/...``.
...
def _normalize_href(value: str) -> str:
    ...
    - relative path ``/en/legal/guidelines-pct/2024/g_ii_3_1.html``
    - absolute URL ``https://www.epo.org/en/legal/guidelines-pct/2024/g_ii_3_1.html``
    """
    h = value.strip()
    ...
    h = re.sub(r"^(?:en/)?legal/guidelines-pct/\d{4}/", "", h)
    h = h.removesuffix(".html")
    return h.lower()


def _build_result_url(base_url: str, version: str, href: str) -> str:
    return f"{base_url}/en/legal/guidelines-pct/{version}/{href}.html"


def _hit_to_model(hit: Any, base_url: str, version: str) -> PctGuidelinesSearchHit:
    title = (
...
class PctGuidelinesClient:
    """Corpus-backed EPO PCT-EPO Guidelines client."""

    DEFAULT_BASE_URL: str = os.getenv("PCT_GUIDELINES_BASE_URL", "https://www.epo.org")
    CACHE_NAME: str = "guidelines"
    DEFAULT_VERSION: str = os.getenv("PCT_GUIDELINES_VERSION", "2024")
    ...
    async def __aenter__(self) -> PctGuidelinesClient:
        return self
    ...
    async def search(
        ...
        *,
        ...
        if not fts_query:
            return PctGuidelinesSearchResponse(
                hits=[], page=page, per_page=per_page, has_more=False
            )
    ...
    async def list_versions(self) -> list[PctGuidelinesVersion]:
        db = self._open()
        meta = db.meta()
